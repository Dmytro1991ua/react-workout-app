...
export const WorkoutsProvider = ({ children }) => {
  ...
  const setLocaleStorage = (workoutData) => localStorage.setItem('workouts', JSON.stringify(workoutData));
  ...
  const runningPace = () => Number((distance / duration).toFixed(1));

  // calculate cycling speed
  const cyclingSpeed = () => Number((distance / (duration / 60)).toFixed(1));
